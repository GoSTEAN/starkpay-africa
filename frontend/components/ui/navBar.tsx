...
export default function NavBar() {
    ...
    const handleScrollToSection = (sectionId: any) => {
        ...
        if (element) {
            const navHeight = 88; // Your navbar height
    ...
    const navItems = [
        ...
        { name: 'How it Works', sectionId: 'values', type: 'scroll' },
        { name: 'Why StarkPay', sectionId: 'why-starkpay', type: 'scroll' },
        { name: 'FAQs', sectionId: 'faq', type: 'scroll' },
    ];
                ...
                >
                    ...
                    {navItems.map((item, index) => (
                        <button
                            ...
                            className="text-[20px] relative z-10 font-[500] hover:bg-white/20 font-[Montserrat] px-4 flex flex-none cursor-pointer"
                        >
                            {item.name}
                        </button>
                    ))}
                            ...
                            <div className="relative z-10 flex flex-col w-full items-start">
                                {navItems.map((item, index) => (
                                    <button
                                        key={index}
                                        onClick={() => handleScrollToSection(item.sectionId)}
                                        className="text-[16px] w-full font-[400] rounded-[30px] hover:bg-white/20 font-[Montserrat] py-2 px-4 flex flex-none text-left"
                                    >
                                        {item.name}
                                    </button>
                                ))}
