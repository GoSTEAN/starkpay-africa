...
export default function Dashboard() {
  ...
  const tabs = [
    { icon: <Home size={25} color="white" className="stroke-3" />, name: "Home" },
    { icon: <QrCode size={25} color="white" className="stroke-3" />, name: "Marchant pay" },
    { icon: <Users size={25} color="white" className="stroke-3" />, name: "Payment split" },
    { icon: <ArrowUpDown size={25} color="white" className="stroke-3" />, name: "Swap" },
    { icon: <Landmark size={25} color="white" className="stroke-3" />, name: "NGN withdrawal" },
    { icon: <History size={25} color="white" className="stroke-3" />, name: "Transactions" },
  ];
      ...
      <div className="bg-[#212324] w-screen justify-between lg:gap-[50px] relative h-screen flex flex-col lg:flex-row lg:items-center px-3 md:px-[50px] lg:px-[200px]">
        ...
        <div className="w-full lg:max-w-[1296px] h-[850px] mt-20 items-start dark-bg-gradient-to-l from-[#8F6DF5]/20 to-[#212324]/90 rounded-2xl overflow-y-scroll">
          ...
          {activeTab === "Marchant pay" && (
            <MarchantPayment onTransaction={addTransaction} addNotification={addNotification} />
          )}
          {activeTab === "Transactions" && <TransactionHistory transactions={notifications} />}
          {activeTab === "NGN withdrawal" && (
            <NGNWithdrawal triggerStatus={(type, status) => { setType(type); setStatus(status); }} addNotification={addNotification} />
          )}
